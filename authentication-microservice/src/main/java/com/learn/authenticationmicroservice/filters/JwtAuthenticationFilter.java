...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    public void doFilterInternal(@NonNull HttpServletRequest request,
        ...
        if (PUBLIC_URLS.contains(path) || (request.getHeader("Authorization") == null || !request.getHeader("Authorization").startsWith("Bearer "))) {
            filterChain.doFilter(request, response);
            return;
        }
        try {
            log.info("***** doFilterInternal: request.getHeader(\"Authorization\"): {}", request.getHeader("Authorization"));
